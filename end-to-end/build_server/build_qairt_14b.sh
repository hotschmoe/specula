#!/bin/bash
# QAIRT no-AIMET w8a16: per part convert -> quantize -> context-bin.
...
SPLIT=$RUN/06_split; DLC=$RUN/07_dlc; QDLC=$RUN/08_qdlc; BIN=$RUN/09_bin; LOG=$RUN/qairt_logs
CFG=/workspace/qnn_v81_box.json
mkdir -p $DLC $QDLC $BIN $LOG
fail=0
for pd in $(ls -d $SPLIT/part*/ | sort -V); do
  p=$(basename $pd)
  echo "===== $p ====="
  python $Q/bin/x86_64-linux-clang/qairt-converter --input_network ${pd}model.onnx --output_path $DLC/${p}.dlc --preserve_io_datatype > $LOG/${p}_convert.log 2>&1
  if [ ! -f $DLC/${p}.dlc ]; then echo "$p CONVERT_FAIL"; tail -4 $LOG/${p}_convert.log; fail=1; continue; fi
  ...
  if [ ! -f $QDLC/${p}.dlc ]; then echo "$p QUANT_FAIL"; tail -4 $LOG/${p}_quant.log; fail=1; continue; fi
  ...
  if [ ! -f $BIN/${p}.bin ]; then echo "$p CTXBIN_FAIL"; tail -4 $LOG/${p}_ctxbin.log; fail=1; continue; fi
  echo "$p OK ($(du -h $BIN/${p}.bin | cut -f1))"
done
echo "QAIRT_BUILD_DONE fail=$fail"
ls -la $BIN/*.bin 2>/dev/null | sed "s|^|  |"

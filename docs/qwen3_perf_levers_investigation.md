...
| Lever C w4a16-local-pr (per-row weight quant) | SOFT PASS (cos 0.888, 100% greedy) | — | — | — | Per-row weight quant fixes the V-projection collapse at w4. Below 0.95 strict gate, but 100% greedy match with CPU reference on 3 multi-steps. Binary 620 MB (32% smaller than per-tensor). AC steady-state 21.4 ms/step (fastest variant). Sweep pending. |

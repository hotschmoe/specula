...
| Lever C local QAIRT pivot (sessions 15-18) | DELIVERED | — | — | — | x86 installed QAIRT 2.42 local toolchain + shotgun of 7 PTQ variants (6 distinct). AI Hub orchestrator bypassed entirely. Full details in `docs/w4a16_investigation.md` + `docs/phase5_local_qairt_compile_findings.md`. |
| Lever C w4a16-local (baseline tf) | NEGATIVE (cos 0.33) | — | — | — | Layer-1+ V-projection weights collapse under w4 precision. Confirmed via session-17 per-layer differential probe (fp16-local oracle vs w4a16-local-tfe: layer-0 V cos 0.957 → layer-1 V cos 0.130, stays <0.2 through layer 27). |
| Lever C w4a16-local-tfe (enhanced act cal) | NEGATIVE (cos 0.36) | — | — | — | Activation calibration moves cos +0.03. Confirms PTQ failure is weight-precision-dominated, not activation-range-dominated. |
| Lever C w4a16-local-mse (mse act cal) | NEGATIVE (cos 0.33) | — | — | — | Same as tf. Cal algorithm ruled out as the lever. |
| Lever C w4a16-local-pr (AC sweep done) | **NEGATIVE AS PRODUCT** | 12.22 | −33% | 54.51% | 40 cells in 18.1 min. Best p8 k=2 = 13.49 t/s / 69.0%. 620 MB binary (32% smaller than w8a16). Per-row quant fixes w4 V-collapse (cos 0.888 + 100% CPU-ref greedy), but **17 pp lower accept vs 8B target than w8a16-local** — CPU-ref greedy-match did NOT predict 8B-target greedy-match. Top-5 overlap (3/5 vs w8a16's 4/5) was the more predictive correctness signal. Slower wall than w8a16 despite faster per-step: lower accept → more rounds → more HTTP verify overhead. CSV: `spec-npu-Qwen3-8B-Q4_K_M-vs-Qwen3-0.6B-pathb-async-pipelined-20260422-224213.csv`. |
| Lever C w8a16-local (AC sweep done) | **NEGATIVE AS PRODUCT** | **12.83** | **−29%** | 71.65% | 40 cells in 14.2 min. Best p2 k=2 = 14.39 t/s / 78.2%. Worst p6 k=8 = 4.73 t/s / 26.9%. Correctness is fine (cos 0.963/0.979, argmax ✓, multi-step 100%). Per-step 23.7 ms ≈ Lever B's pathbmask fp16 (22-25 ms). The ~10 pp accept-rate drop from PTQ noise dominates the throughput equation. CSV: `spec-npu-Qwen3-8B-Q4_K_M-vs-Qwen3-0.6B-pathb-async-pipelined-20260422-222705.csv`. |
| Lever C w8a16-local-pr | SOFT PASS (cos 0.924) | — | — | — | Per-row quant hurts at w8 (precision already sufficient; extra scale metadata doesn't help). Not a winner. |
| Lever C fp16-local | REFERENCE (cos 0.9999) | — | — | — | No PTQ (`qairt-quantizer --float_bitwidth 16`). Numerically exact; 49.8 ms/step (2× slower than quantized). Diagnostic only — no per-step win over fp16 pathbmask. |

...
So the work is worth doing. Order:

1. **Decide local-rewrite vs AI Hub.** If `qai-hub-models` already has
   a `gemma4` recipe (check: `python submit_ai_hub.py --check` with a
   token), Path A may produce a bundle with zero graph surgery — try
   that first. If not, the local pipeline is the path.
2. **Download `google/gemma-4-E2B-it`** and run
   `quantize_to_npu.py --dry-run` to confirm the model-config resolves.
3. **Build the four `scripts/` rewrites**, validating each at cos ≥
   0.99 with `probe_gemma4_equivalence.py`. Budget: PLE + partial RoPE
   are ~1 session each; the htp/pin scripts are lighter ports.
4. **Rent a RunPod A100**, run stages 6–9. ~$3–10, ~3–8 h wall.
5. **Validate on the Snapdragon laptop** via `npu_engine`.

## Open questions

- Does `qai-hub-models` have a Gemma 4 recipe yet? (decides 1)
- Does `optimum-cli` export the text decoder cleanly past the
  multimodal wrapper? (decides whether script 4 is needed)

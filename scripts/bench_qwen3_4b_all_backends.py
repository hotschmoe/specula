"""One-call bench driver for the Qwen3-4B all-backends baseline matrix.

Runs one or more backends, captures PP512 + TG (tokens/sec), and — on
battery — measures Joules/token via WMI RemainingCapacity delta across
each backend's run.
...
Usage:
    ...
    .venv/Scripts/python.exe scripts/bench_qwen3_4b_all_backends.py \\
        --power-state ac \\
        --backends npu-genie,cpu \\
        --tag retry

Outputs (to results/qwen3_4b_baseline/):
    bench_<tag>.csv           — machine-readable results
    bench_<tag>.md            — markdown-ready table rows
    <backend>_<tag>.log       — raw stdout/stderr per backend
"""
...
GGUF_PATH = REPO_ROOT / "models" / "Qwen3-4B-Q4_K_M.gguf"
...
def run_llama_bench(
    ...
    # llama-bench JSON: array of run dicts. Keys include "test" (e.g. "pp512"
    # or "tg128") and "avg_ts" (tokens/sec, averaged over -r runs).
    try:
        data = json.loads(proc.stdout)
    except json.JSONDecodeError as e:
        result.ok = False
        result.notes = f"json parse: {e}"
        return result
...
def write_md(md_path: Path, rows: list[dict], power_state: str, tag: str) -> None:
    lines = [
        f"# Qwen3-4B baseline — {power_state.upper()} — {tag}",
        "",
        f"Generated by `scripts/bench_qwen3_4b_all_backends.py --power-state {power_state} --tag {tag}` on {time.strftime('%Y-%m-%d %H:%M:%S')}.",
        "",
    ...
    for r in rows:
        ...
        lines.append(
            ...
            f"{r['tg_tokens']} | {fmt(r['mwh_drop'], '.0f')} | "
...
def main() -> int:
    ...
    for name, fn, kwargs in selected:
        ...
        log_path = OUT_DIR / f"{name}_{tag}.log"
        ...
        # J/tok math: only meaningful on battery, and only over a window
        # that actually drained measurable mWh. We use prefill + decode
        # tokens together (total compute work during the drop).
        j_per_tok = None
        mwh_drop = None
        if mwh_before is not None and mwh_after is not None:
            ...
            if mwh_drop > 0 and total_tokens > 0:
                energy_j = mwh_drop * 3.6  # mWh -> J
        ...
        print(f"  wall: {wall:.1f}s  mWh drop: {mwh_drop}  J/tok: {j_per_tok}")
        if not result.ok:
            print(f"  !! NOT OK: {result.notes}")

    csv_path = OUT_DIR / f"bench_{tag}.csv"
    md_path = OUT_DIR / f"bench_{tag}.md"
    write_csv(csv_path, rows)
    write_md(md_path, rows, args.power_state, tag)
    print(f"\n=== Summary ===")

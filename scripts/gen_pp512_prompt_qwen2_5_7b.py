...
under the Qwen2.5-7B-Instruct tokenizer.

Writes:
    results/qwen2_5_7b_baseline/pp512_prompt.txt        - chat-wrapped text
    results/qwen2_5_7b_baseline/pp512_prompt_tokens.txt - one token id per line

Prefers the bundle's tokenizer.json once the AI Hub Workbench compile
lands; falls back to results/qwen2_5_7b_baseline/tokenizer_upstream.json
(a cached copy of Qwen/Qwen2.5-7B-Instruct/tokenizer.json from HF —
fetch with curl while the bundle is in flight). Both files are byte-
identical because Qualcomm doesn't modify tokenizers; this lets us
generate the prompt before the bundle exists.

Run:
    .venv/Scripts/python.exe scripts/gen_pp512_prompt_qwen2_5_7b.py
"""
...
BUNDLE_TOK = (
    ...
    / "qwen2_5_7b_instruct-genie-w8a16-qualcomm_snapdragon_x2_elite" / "tokenizer.json"
)
UPSTREAM_TOK = REPO_ROOT / "results" / "qwen2_5_7b_baseline" / "tokenizer_upstream.json"
TOKENIZER_PATH = BUNDLE_TOK if BUNDLE_TOK.exists() else UPSTREAM_TOK
OUT_DIR = REPO_ROOT / "results" / "qwen2_5_7b_baseline"
